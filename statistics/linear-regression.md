...
如何找到high leverage？可以通过观察x，但有些情况对于单个变量是合理的，但组合在一起就不行了，如右图。有一种量化leverage的方法，leverage statistic。对于一元线性回归， $$h_i=\frac{1}{n}+\frac{(x_i-\bar{x})^2}{\sum(x_{i'}-\bar{x})^2}$$。可见 $$h_i$$随着 $$x_i-\bar{x}$$的增加而增加。对于多元回归，$$h_i$$总是在 $$(\frac{1}{n},1)$$之间，而且所有x的平均杠杆值总是等于$$\frac{p+1}{n}$$，因此如果某个leverage大大超过这个平均值，对应的x很有可能具有较高的杠杆作用。
...
**共线性的缺点**：除了难以分离出单个变量对响应值的影响，另外对于不同变量x对应系数 $$\beta$$的RSS等高线图形成狭窄的图形。我们希望等高线图是一个圆形或者椭圆，越接近中心，RSS值越低，而且系数对应的取值范围也比较合理。但对于共线性问题，一旦图形变得狭长，数据微小变化可能导致RSS的估计有很大不确定性，从而降低了回归系数估计的准确性，进而导致t-test下降，有可能会拒绝该系数。
